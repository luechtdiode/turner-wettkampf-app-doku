...
Im GeTu ist es bei geschlechts-gemischten Durchgängen üblich, dass die Barren-Station am Ende eines Durchgangs für alle Turner aus dem Durchgang durchgeführt wird. Die automatische Einteilung kann die Barren-Riege in folgenden Varianten einteilen:

1. Mit der Checkbox angewählt, dass die `Riege2 in einen separaten Durchgang eingeteilt` wird, wo dann die Barren-Riegen zugewiesen sind.
2. Ohne dediziertem Durchgang für die Riege2-Zuteilung, aber in dem Durchgang als `Riege2 Gerät` integriert, so dass der Barren separat und nicht in die Rotation aller anderen Geräte kommt.
3. Mit der Checkbox in der `Startgeräteliste beim Barren aktiviert`, wird der Barren in die normale Startgeräte-Rotation im selben Durchgang eingeteilt. Diese Einstellung ist vorbelegt bei reinen Männer-Wettkämpfen.

Diese Einteilung lässt sich aber auch manuell mit folgendem Trick erreichen:
